...
def write_log(w):
    file_name = config.sys_path+'/data/' + datetime.date.today().strftime('%m%d') + "_{}.log".format("bert_base_loss")
...
from train import train_one_epoch_loss
def train(epoch_num):
    min_test_epoch_loss=999999
    for i in range(epoch_num):
        write_log("Current lr : {}".format(optimizer.state_dict()['param_groups'][0]['lr']))
        min_test_epoch_loss=train_one_epoch_loss(config.sys_path+'/data/bert_loss/',min_test_epoch_loss,device0,device1,
                                            i,train_loader,test_loader,
                                            write_log,cls,optimizer,
                                            test_epoch_loss_l,test_acc_l,train_epoch_loss_l,train_acc_l,lmd=20)
        scheduler.step()
